...
const COPY = {
  en: {
    ...
    selectedProof: "Selected capability proof",
    open: "Open",
  ...
  tr: {
    ...
    selectedProof: "Seçili capability kanıtı",
    open: "Aç",
...
function selectedCard(number) {
  return document.getElementById(`capability-${number}-status`)?.closest(".capability-card") ?? null;
}

function statusText(number) {
  return document.getElementById(`capability-${number}-status`)?.textContent?.trim() ?? "";
}
...
function makeWorkspace(activeActions) {
  ...
  stageLabel.append(node("span", "", lang() === "tr" ? "CAPABILITY WORKSPACE" : "CAPABILITY WORKSPACE"), node("strong", "workspace-stage-status", ""));
...
function proofNode(className) {
  return shell?.querySelector(`.${className}`) ?? null;
}
...
function setProofItem(className, { id = t("unavailable"), meta = "", ready = false, href = null } = {}) {
  ...
  const oldLink = body.querySelector("a.proof-open-link");
  oldLink?.remove();
  if (ready && href) {
...
async function syncProof(number) {
  ...
  setProofItem("proof-certificate", {
    id: certId ? short(certId, 28) : t("unavailable"),
    meta: certified ? t("verified") : t("waiting"),
    ready: Boolean(certId),
    href: certId ? `/certificate/${encodeURIComponent(certId)}` : null,
  });
  setProofItem("proof-profile", {
    id: `Capability ${number}`,
    meta: certified ? t("profileActive") : t("waiting"),
    ready: certified,
  });
  ...
  try {
    ...
    if (!receiptId) return;
    setProofItem("proof-receipt", {
      id: short(receiptId, 28),
      meta: lang() === "tr" ? "Signed verification receipt" : "Signed verification receipt",
      ready: true,
      href: `/verify/${encodeURIComponent(receiptId)}`,
    });
    ...
    setProofItem("proof-public", {
      id: `proof · ${short(receiptId, 19)}`,
      meta: valid ? `${verdict || "PASS"} · ${t("portable")}` : t("portable"),
      ready: valid,
      href: `/verify/${encodeURIComponent(receiptId)}`,
    });
  } catch {
    // Proof cards remain grounded in already-known certificate state when the public API is unavailable.
  }
}
...
function syncSelector() {
  ...
  shell.querySelectorAll(".capability-selector-button").forEach((button) => {
    ...
    const marker = isCertified(number) ? " ✓" : isVerifying(number) ? " ·" : "";
    button.textContent = `C${number}${marker}`;
  });
}
...
function syncAgentSummary() {
  ...
  const count = certificateText.match(/\d+/)?.[0] ?? "0";
  shell.querySelector(".shell-cert-count").textContent = count;
...
function selectCapability(number, { scroll = false } = {}) {
  ...
  const card = selectedCard(number);
  card?.classList.add("is-workspace-active");
  syncShell();
...
function refreshLocalization() {
  ...
  const newSidebar = makeSidebar();
  const newProof = makeProofPanel();
  oldSidebar?.replaceWith(newSidebar);
  oldProof?.replaceWith(newProof);
...
if (activeActions) {
  ...
  observer.observe(activeActions, { attributes: true, childList: true, subtree: true, characterData: true });
}

const identitySummary = document.getElementById("identity-summary");
if (identitySummary) {
  const observer = new MutationObserver(maybeActivate);
  observer.observe(identitySummary, { attributes: true, childList: true, subtree: true });
}
